...
#"condor_rm $USER" removes your jobs 
  ...
  echo "#!/bin/bash" > npx3-execs/npx3-$3-$$.sh
  echo "date" >> npx3-execs/npx3-$3-$$.sh
  echo "hostname" >> npx3-execs/npx3-$3-$$.sh
  echo "cd `pwd`" >> npx3-execs/npx3-$3-$$.sh
  #Set up new tools
  echo "eval `/cvmfs/icecube.opensciencegrid.org/py2-v3.0.1/setup.sh`" >> npx3-execs/npx3-$3-$$.sh
  echo "$@" >> npx3-execs/npx3-$3-$$.sh
  echo "date" >> npx3-execs/npx3-$3-$$.sh


  chmod +x npx3-execs/npx3-$3-$$.sh

  # Creating condor submission script (ClassAd)
  echo "Universe  = vanilla" > 2sub.sub
  echo "Executable = npx3-execs/npx3-$3-$$.sh" >> 2sub.sub
  echo "Log = npx3-logs/npx3-$3-\$(cluster).log" >> 2sub.sub
  echo "Output = npx3-out/npx3-$3-\$(cluster).out" >> 2sub.sub
  echo "Error = npx3-error/npx3-$3-\$(cluster).error" >> 2sub.sub
  ...
  echo "request_disk = 1GB" >> 2sub.sub
  echo "Request_memory = 1 GB" >> 2sub.sub
